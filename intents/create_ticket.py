...
def post_data(req):
    ...
    issue_list = [name['parameters'].get('issue_sum') for name in contexts
                 if name['parameters'].get('issue_sum') is not None]

    issue_sum = issue_list[0]
